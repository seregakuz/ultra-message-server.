...
// "Онлайн" — если был активен совсем недавно (учитывая, что клиент опрашивает
// сервер каждые ~3 секунды, пока открыт активный чат).
const ONLINE_WINDOW_MS = 20 * 1000;
function publicUser(u){
  // То, что можно безопасно показать другим пользователям (без пароля/соли)
  const online = !u.hideLastSeen && !!u.lastActiveAt && (Date.now() - u.lastActiveAt < ONLINE_WINDOW_MS);
  return {
    id: u.id, name: u.name, username: u.username,
    phone: u.phonePrivate ? null : u.phone,
    phonePrivate: !!u.phonePrivate,
    bio: u.bio || '', colorIndex: u.colorIndex || 0,
    avatarPhoto: u.hideAvatar ? null : (u.avatarPhoto || null),
    online,
    lastSeenAt: u.hideLastSeen ? null : (u.lastActiveAt || null)
  };
}
function meUser(u){
  // Полная версия для самого владельца аккаунта (телефон и аватар всегда видны себе)
  return {
    ...
    hideAvatar: !!u.hideAvatar, hideLastSeen: !!u.hideLastSeen, hideTypingIndicator: !!u.hideTypingIndicator
  };
}
...
// "Печатает..." — тоже только в памяти: сигнал живёт несколько секунд и обновляется,
// пока человек печатает. Хранить это в db.json смысла нет.
const TYPING_TTL_MS = 6000;
const typingSignals = new Map(); // "fromId->toId" -> истекает в это время
function setTyping(fromId, toId){
  typingSignals.set(fromId + '->' + toId, Date.now() + TYPING_TTL_MS);
}
function isTyping(fromId, toId){
  const expires = typingSignals.get(fromId + '->' + toId);
  return !!expires && expires > Date.now();
}
setInterval(() => {
  const now = Date.now();
  for (const [key, expires] of typingSignals) {
    if (expires < now) typingSignals.delete(key);
  }
}, 60 * 1000).unref();
...
function findUserByToken(token){
  ...
  // Только в памяти — не дёргаем saveDb() на каждый запрос, это попадёт на диск
  // само собой при следующем сохранении по любой другой причине.
...
route('POST', '/api/register', async (req, res) => {
  ...
  const user = {
    ...
    hideAvatar: false, hideLastSeen: false, hideTypingIndicator: false,
...
route('PUT', '/api/me/profile', async (req, res) => {
  ...
  if (typeof body.hideAvatar === 'boolean') user.hideAvatar = body.hideAvatar;
  if (typeof body.hideLastSeen === 'boolean') user.hideLastSeen = body.hideLastSeen;
  if (typeof body.hideTypingIndicator === 'boolean') user.hideTypingIndicator = body.hideTypingIndicator;
  saveDb();
...
route('GET', '/api/dm/:otherId', async (req, res, query, params) => {
  ...
  // так насобирать данные всех пользователей вообще без переписки с ними. Отдаём только
  // три узких, приватность-совместимых поля о статусе собеседника — не весь профиль.
  const otherOnline = !other.hideLastSeen && !!other.lastActiveAt && (Date.now() - other.lastActiveAt < ONLINE_WINDOW_MS);
  send(res, 200, {
    messages,
    otherTyping: !other.hideTypingIndicator && isTyping(otherId, user.id),
    otherOnline,
    otherLastSeenAt: other.hideLastSeen ? null : (other.lastActiveAt || null)
  });
});

route('POST', '/api/dm/:otherId/typing', async (req, res, query, params) => {
  const user = findUserByToken(getToken(req));
  if (!user) return send(res, 401, { error: 'unauthorized' });
  if (isRateLimited('typing:' + user.id, 60, 5 * 60 * 1000)) {
    return send(res, 429, { error: 'rate_limited' });
  }
  const otherId = parseInt(params.otherId, 10);
  if (!db.users.some(u => u.id === otherId)) return send(res, 404, { error: 'not_found' });
  setTyping(user.id, otherId);
  send(res, 200, { ok: true });
});
